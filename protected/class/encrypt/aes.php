...
class Aes {
	 ...
	 * @param string $iv iv向量, 如 md5(time().uniqid(), true)
	 * @param mixed $options 还不是很清楚
	 *
	 */
	public function __construct($key, $method = 'AES-128-ECB', $iv = '', $options = 0) {
		// key是必须要设置的
		$this->secret_key = isset($key) ? $key : 'MARIO_@AES_@20190916';
		$this->method = $method;
		$this->iv = $iv;
	...
	public function encrypt($data) {
		$encrypt = openssl_encrypt($data, $this->method, $this->secret_key, $this->options, $this->iv);
		$encrypt = str_replace('/', '|', $encrypt);
		return rawurlencode($encrypt);
	}
