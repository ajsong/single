...
class Encipher
{
    ...
    /**
     * advanced encryption, 加密模式,低级模式不使用eval函数,高级模式使用了eval函数
     */
    ...
    public function __construct($source_file = '', $encoded_file = '', $comments = array(), $exclude_path = array()) {
	    ...
	    if (!is_dir($this->source_file) && !file_exists($this->source_file)) {
		    exit('Source path/file does not exist.');
	    }
	    if (empty($this->encoded_file)) {
		    exit('Encoded path/file parameter does empty.');
	    }
    }
    ...
    private function _encryptFiles($files) {
        foreach ($files as $file) {
	        if (stripos($file, '.DS_Store') !== false) {unlink($file);continue;}
	        if (basename($file) == 'encipher.php') continue;
	        $is_encode = true;
	        ...
	        $code = file_get_contents($this->source_file . '/' . $file);
	        if (strpos($code, '//Compile code')) {
		        copy($this->source_file . '/' . $file, $this->encoded_file . '/' . $file);
		        continue;
	        }
    ...
    private function _setHumanUnreadable($file) {
	    ...
	    $headers = array('<?php', '//Compile code');
	    if(count($this->comments))$headers = array_merge($headers, array_map('trim', array_merge(array('/*'), $this->comments, array('*/'))));
