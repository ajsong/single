...
class Wxpay_client_pub extends Common_util_pub
{
	...
	function postXml()
	{
		...
		if ($this->debug) $this->log($this->response);
 ...
 * 企业付款到零钱接口类
 */
class TransfersOrder_pub extends Wxpay_client_pub
{
	function __construct() {
		parent::__construct();
		//设置接口链接
	...
	function createXml()
	{
		try
		{
			...
			{
				throw new SDKRuntimeException("缺少企业付款到零钱接口必填参数partner_trade_no！"."<br>");
			}elseif($this->parameters["openid"] == null){
				throw new SDKRuntimeException("缺少企业付款到零钱接口必填参数用户openid！"."<br>");
			}elseif ($this->parameters["amount"] == null ) {
				throw new SDKRuntimeException("缺少企业付款到零钱接口必填参数amount！"."<br>");
			}elseif ($this->parameters["desc"] == null) {
				throw new SDKRuntimeException("缺少企业付款到零钱接口必填参数desc！"."<br>");
			}
			$this->parameters["mch_appid"] = $this->config->APPID;//公众账号ID
			$this->parameters["mchid"] = $this->config->MCHID;//商户号
			$this->parameters["check_name"] = 'NO_CHECK';//校验用户姓名选项
			$this->parameters["spbill_create_ip"] = $this->getIP();//终端ip
			$this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
			$this->parameters["sign"] = $this->getSign($this->parameters);//签名
			if ($this->debug) $this->log(json_encode($this->parameters));
			return  $this->arrayToXml($this->parameters);
...
class UnifiedOrder_pub extends Wxpay_client_pub
{
	...
	function getIP(){
		...
		//if ($ip=='::1' || $ip=='127.0.0.1') $ip = $this->getIPTaobao();
	...
	function getPrepayId()
	{
		...
		if(isset($this->result["prepay_id"])){
			...
			if (function_exists('error')) {
				$msg = '';
				if(isset($this->result['err_code_des'])) $msg = $this->result['err_code_des'];
				if(isset($this->result['return_msg'])) $msg = $this->result['return_msg'];
				if (strlen($msg)) error($msg);
			}
			$prepay_id = '';
		}
		return $prepay_id;
	...
	public function getParametersForAPP($prepay_id, $appId='')
	{
		$timeStamp = time();
		$jsApiObj["appid"] = strlen($appId) ? $appId : $this->config->APPID;
		$jsApiObj["partnerid"] = $this->config->MCHID;
...
class Refund_pub extends Wxpay_client_pub
{
	...
	function createXml()
	{
		try
		{
			$out_trade_no = (isset($this->parameters["out_trade_no"]) && $this->parameters["out_trade_no"]) ? $this->parameters["out_trade_no"] : '';
			$transaction_id = (isset($this->parameters["transaction_id"]) && $this->parameters["transaction_id"]) ? $this->parameters["transaction_id"] : '';
			//检测必填参数
...
class JsApi_pub extends Common_util_pub
{
	 ...
	 * 	作用：设置jsapi的参数
	 */
	public function getParameters($appId='')
	{
		$timeStamp = time();
		...
		$jsApiObj["appId"] = strlen($appId) ? $appId : $this->config->APPID;
		$jsApiObj["timeStamp"] = "$timeStamp";
		$jsApiObj["nonceStr"] = $nonceStr;
		$jsApiObj["package"] = "prepay_id=$this->prepay_id";
		$jsApiObj["signType"] = "MD5";
		
		if ($this->miniprogram) {
			$returnApiObj["timeStamp"] = "$timeStamp";
			$returnApiObj["nonceStr"] = $nonceStr;
			$returnApiObj["package"] = "prepay_id=$this->prepay_id";
			$returnApiObj["signType"] = "MD5";
			$returnApiObj["paySign"] = $this->getSign($jsApiObj);
			$this->parameters = json_encode($returnApiObj);
		} else {
