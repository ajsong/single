...
class PHPZip
{
	...
	public function ZipAndDownload($dir)
	{
		...
		@header('Content-Disposition: attachment ; filename=Zip'.date("YmdHis").'.zip');
	...
	private function ExtractFile($header, $to, $zip)
	{
		...
		if(substr($to, -1) != "/"){ $to .= "/"; }
		if(!@is_dir($to)){ $this->makeDir($to); }
		 
		$pth = explode("/", dirname($header['filename']));
		for($i=0; isset($pth[$i]); $i++){
			if(!$pth[$i]){ continue; }
			$pthss .= $pth[$i]."/";
			if(!is_dir($to.$pthss)){ $this->makeDir($to.$pthss); }
		}
