			...
			<ul class="list" v-if="data && $.isArray(data.goods) && data.goods.length">
				<li v-for="g in data.goods">
					<router-link :to="{path:'/goods/detail', query:{id:g.id}}">
						...
						<div class="content">
							<div class="purchase">
								<div :style="{width:$.round(g.purchase_count/g.purchase_amount*100, 1)+'%'}"></div>
								<span>已抢{{ (g.purchase_count/g.purchase_amount*100)|round(1) }}%</span>
							</div>
						</div>
						<font class="btn"><b>立即秒杀</b></font>
						<span class="price"><strong>￥{{ g.purchase_price|round }}</strong><s>￥{{ g.market_price|round }}</s></span>
					</router-link>
				</li>
			</ul>
...
export default {
	...
	methods:{
		...
		getData(page, scroller){
			...
			this.$ajax.get('/api/goods/purchase', params).then(json => {
