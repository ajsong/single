...
<div class="goods-index">
	<pull-refresh :refresh="refresh" :loadmore="loadmore" :init="scrollerInit">
		<header :class="{'brandHeader':brand_id>0}" v-if="integral!==1 && groupbuy!==1">
			...
			<switch-view class="ge-bottom ge-light" :data="switchData" column="column"></switch-view>
		</header>
...
export default {
	...
	created(){
		this.func = this.$.storageJSON('func')
	...
	methods: {
		switchSelected(index){
			if (index < this.switchData.length - 1) {
				...
				this.$(this.$refs.filter).presentView(1)
				return false
			}
		},
		reset(){
		...
		search(){
			this.$(this.$refs.filter).presentView(false)
			this.params = {
		...
		getData(page, scroller){
			let params = this.$.extend(this.params, {
			...
			this.$ajax.get('/api/goods', params).then(json => {
				...
				this.$nextTick(() => {
					...
					if (isInit && json.data.brand) {
						this.$(this.$refs.brandbanner).loadbackground('url', '50%', '../images/nopic.png')
						this.$(this.$refs.brandpic).loadbackground('url', '50%', '../images/nopic.png')
					}
					this.$(this.$refs.pic).caller(this).loadbackground('url', '50%', '../images/nopic.png')
				})
			})
		}
	}
