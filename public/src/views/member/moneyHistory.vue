...
<div>
	<div class="navBar">
		...
		<div class="titleView-x">余额明细</div>
	</div>
		...
		<pull-refresh :refresh="refresh" :loadmore="loadmore" :init="scrollerInit" :marginTop="$.toPx(0.44)">
			<ul class="list" v-if="data">
				<li class="ge-bottom ge-light" v-for="g in data">
					<div :class="{'r':String(g.money).indexOf('-')>-1}">{{ g.money|round }}</div>
					<span>{{ g.memo }}</span>
...
export default {
	...
	methods:{
		...
		getData(page, scroller){
			this.$ajax.get('/api/recharge/history', {offset:(page.num - 1) * page.size, pagesize:page.size}).then(json => {
