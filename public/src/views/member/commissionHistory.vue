		...
		<pull-refresh :refresh="refresh" :loadmore="loadmore" :init="scrollerInit" :marginTop="$.toPx(0.44)">
			<ul class="list" v-if="data">
				<li class="ge-bottom ge-light" v-for="g in data">
					<div :class="{'r':String(g.commission).indexOf('-')>-1}">{{ g.commission|round }}</div>
					<span>{{ g.memo }}</span>
...
export default {
	...
	methods:{
		...
		getData(page, scroller){
			this.$ajax.get('/api/member/commission_history', {offset:(page.num - 1) * page.size, pagesize:page.size}).then(json => {
