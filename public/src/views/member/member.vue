...
<div>
	...
	<div class="member-index main-bottom width-wrap">
		<div class="topView">
			<div class="infoView">
				<div class="avatar" v-if="member.id>0 && member.avatar.length" :style="{'background-image':'url('+member.avatar+')'}"></div>
		...
		<section>
			<ul class="tableView tableView-light">
				...
				<li class="orderList">
					<router-link class="ico1 badge" to="/wap/?app=order&act=index&status=0"><div><sub v-html="data.not_pay>0?data.not_pay:''"></sub></div></router-link>
					<router-link class="ico2 badge" to="/wap/?app=order&act=index&status=1"><div><sub v-html="data.not_shipping>0?data.not_shipping:''"></sub></div></router-link>
					<router-link class="ico3 badge" to="/wap/?app=order&act=index&status=2"><div><sub v-html="data.not_confirm>0?data.not_confirm:''"></sub></div></router-link>
					<router-link class="ico4 badge" to="/wap/?app=order&act=index&status=3"><div><sub v-html="data.not_comment>0?data.not_comment:''"></sub></div></router-link>
				</li>
			</ul>
			...
			<ul class="tableView tableView-light ge-top ge-light">
				<li v-if="inArray('coupon',func)"><router-link to="/coupon?status=1"><h1><big v-if="data.coupon_count>0">{{ data.coupon_count }}张</big><em class="ico4"></em>我的优惠券</h1></router-link></li>
...
export default {
	...
	created(){
		this.func = JSON.parse(this.$.storage('func'))
		this.edition = Number(this.$.storage('edition'))
		this.member = JSON.parse(this.$.storage('member'))
