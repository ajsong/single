	...
	<div>
		...
		<div class="home-index main-padding-bottom">
			<pull-refresh :refresh="refresh" :loadmore="loadmore" :init="scrollerInit" :marginTop="$.toPx(0.44+0.49)">
				...
				<div class="cate" v-if="inArray('category', func)">
					<router-link class="groupbuy" to="/goods/groupbuy" v-if="inArray('groupbuy', func)"><div></div><span>特价拼团</span></router-link>
					<router-link class="purchase" to="/goods/purchase" v-if="inArray('purchase', func)"><div></div><span>限时秒杀</span></router-link>
					<router-link class="chop" to="/goods/chop" v-if="inArray('chop', func)"><div></div><span>限量砍价</span></router-link>
	...
	methods: {
		...
		getData(page, scroller){
			this.$ajax.get('/api', {offset:(page.num - 1) * page.size, pagesize:page.size}).then(json => {
				...
				if(!this.data){
					this.$.storage('func', json.function)
					this.$.storage('edition', json.edition)
					this.func = json.function
					this.data = json.data
				}
				if (!(json.data.recommend instanceof Array)) return scroller.end()
				let data = page.num === 1 ? [] : this.data.recommend
				data.push(...json.data.recommend)
				this.data.recommend = data
				this.$nextTick(() => {
					scroller.end()
					//this.$(this.$refs.pic).loadbackground('url', item => item.getAttribute('data-name')==='category' ? '100%' : '50%', '../images/nopic.png')
					this.$refs.pic.forEach(item => {
						if (!item.getAttribute('url')) return
						this.$(item).loadpic(item.getAttribute('url'), '../images/nopic.png', (item, pic, state) => {
