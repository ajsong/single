...
<div ref="scroller" class="pullRefresh">
	<div ref="wraper" class="pull-wrap" :style="{height:fullHeight ? '100%' : ''}">
		...
		<div ref="footer" class="pull-footer" v-if="isFooterCreated && loadmoreParams.callback">
			...
			<div class="pull-nomore" v-else-if="loadmoreParams.noMoreText">{{ loadmoreParams.noMoreText }}</div>
			</template>
		</div>
	</div>
</div>
...
export default {
	...
	methods:{
		...
		setPageSize(num){
			this.loadmoreParams.size = Number(num)
		},
		scrollTop(top){
		...
		transform(){
			...
			if (transform === 'none') return { scale: 0, rotate: 0, translate: { x: 0, y: 0 } }
			let matcher = transform.split('(')[1].split(')')[0].split(',')
		...
		startDrag(e){ //顶部下拉监听
			...
			this.transformY = this.transform().translate.y
		...
		moveDrag(e){
			...
			if (this.distance > 0) {
				...
				if (this.distance >= this.refreshHeight) {
					...
					if (this.distance >= 10) this.isMoved = true
					this.refreshText = this.refreshParams.text
		...
		refreshNow(){ //开始无动画刷新
			if (!this.refreshParams.callback || this.loading) return false
			this.isRefresh = true
			this.loading = true
			this.pagenum = 1
		...
		loadmoreBegin(){ //开始加载
			...
			if (!this.loadmoreParams.callback || this.loading) return
			this.loading = true
			this.loadmoreText = this.loadmoreParams.updatingText
			this.pagenum++
			this.loadmoreParams.callback({num:this.pagenum, size:this.loadmoreParams.size}, this)
		},
		end(notUpdateTime){ //结束刷新/加载
			...
			this.iconRotate = false
			if (this.isRefresh) {
				if (notUpdateTime) this.isRefresh = false
				this.scrollTop(0)
				this.refreshText = this.refreshParams.text
			...
			setTimeout(() => {
				...
				this.fullHeight = scrollHeight === 0
				this.loading = false
				this.noMoreData = scrollHeight <= 0 || this.prevScrollHeight >= scrollHeight
				this.fullHeight = scrollHeight === 0
				if (this.loadmoreParams.callback) {
					this.isFooterCreated = scrollHeight > 0
					setTimeout(() => this.prevScrollHeight = this.scrollHeight(), 100)
				}
				if (!notUpdateTime) {
					let getNow = () => {
						let preZero = (str, prec) => (Array(prec).join('0') + '' + str).slice(-prec)
						let now = new Date()
						return now.getFullYear() + '-' + preZero(now.getMonth()+1, 2) + '-' + preZero(now.getDate(), 2) + ' ' +
							preZero(now.getHours(), 2) + ':' + preZero(now.getMinutes(), 2) + ':' + preZero(now.getSeconds(), 2)
					}
					if (this.isRefresh) {
						...
						if (this.prevScrollHeight >= scrollHeight) {
