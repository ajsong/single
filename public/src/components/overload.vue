...
export default {
	...
	watch: {
		...
		text: function(newVal, oldVal){
			...
			if (!this.text) {
				this.imageStyle = $.extend(this.imageStyle, {'margin-top':((view.clientHeight - div.clientHeight) / 2) + 'px'})
				this.textShow = false
			} else {
				this.imageStyle = $.extend(this.imageStyle, {'margin-top':'0.2rem'})
				this.textShow = true
			}
		},
	...
	methods: {
		updatedInterface(){
			...
			setTimeout(() => this.viewClass = 'load-view-in', 50)
			if (this.auto) {
				this.timer = setTimeout(() => {
					this.$emit('overload', false)
				}, this.auto)
