	...
	<div class="slide">
		<ul ref="ul" :style="{width:(dir===0?(imgWidth*(imgList.length+1)):imgWidth)+'px', height:(dir!==0?(imgHeight*(imgList.length+1)):imgHeight)+'px', 'transition-duration':moveSpeed+'ms', left:left, top:top}">
			...
			<li v-for="g in imgList" :style="{width:imgWidth+'px', height:parentHeight}">
				...
				<router-link v-else ref="pic" :to="g.link" :url="g.pic"><span v-if="titleField && g[titleField]">{{ g[titleField] }}</span></router-link>
			</li>
			</template>
			<template v-else>
			<li class="slide-li" v-for="g in imgList" :style="{width:imgWidth+'px', height:parentHeight}">
				<router-link v-if="g.ad_type === 'html5'" ref="pic" :type="g.ad_type" :to="g.ad_content" :url="g.pic"></router-link>
				<router-link v-else-if="g.ad_type === 'goods'" ref="pic" :type="g.ad_type" :to="{path:'/goods/detail', query:{id:g.ad_content}}" :url="g.pic"></router-link>
				<router-link v-else-if="g.ad_type === 'shop'" ref="pic" :type="g.ad_type" :to="{path:'/shop/detail', query:{id:g.ad_content}}" :url="g.pic"></router-link>
				<router-link v-else-if="g.ad_type === 'article'" ref="pic" :type="g.ad_type" :to="{path:'/article/detail', query:{id:g.ad_content}}" :url="g.pic"></router-link>
				<router-link v-else-if="g.ad_type === 'type' || g.ad_type === 'subtype'" ref="pic" :type="g.ad_type" :to="{path:'/goods', query:{category_id:g.ad_content}}" :url="g.pic"></router-link>
				<router-link v-else-if="g.ad_type === 'brand'" ref="pic" :type="g.ad_type" :to="{path:'/goods', query:{brand_id:g.ad_content}}" :url="g.pic"></router-link>
				<router-link v-else-if="g.ad_type === 'country'" ref="pic" :type="g.ad_type" :to="{path:'/goods', query:{country_id:g.ad_content}}" :url="g.pic"></router-link>
				<a v-else-if="g.ad_type === 'coupon'" ref="pic" :type="g.ad_type" href="javascript:void(0)" :url="g.pic" @click="getCoupon(g.ad_content)"></a>
				<router-link v-else-if="g.ad_type === 'recharge'" ref="pic" :type="g.ad_type" :to="{path:'/recharge/commit', query:{id:g.ad_content}}" :url="g.pic"></router-link>
				<router-link v-else-if="g.ad_type === 'register'" ref="pic" :type="g.ad_type" :to="($.storage('member') ? '/member' : '/register')" :url="g.pic"></router-link>
				<a v-else ref="pic" :type="g.ad_type" href="javascript:void(0)" :mid="g.ad_content" :url="g.pic"></a>
			</li>
			</template>
		</ul>
	</div>
		...
		height(){
			if (typeof this.height === 'string') {
				...
				if (/px$/.test(this.height)) {
					...
					this.imgHeight = $.toPx(Number(this.height.replace(/rem/, '')) || 0)
				}
			} else if (typeof this.height === 'number') {
				this.parentHeight = this.height + 'px'
	...
	mounted(){
		...
		this.$nextTick(() => {//页面渲染完成执行
			...
			setTimeout(() => {
				...
				$('.slide-li a').loadbackground('url', '30%', '../../images/nopic.png')
			}, 50)
			this.continueAuto()
