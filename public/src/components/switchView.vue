...
<ul class="switchView" v-if="data.length">
	<div ref="wrap">
		<li v-for="(item, i) in data" :ref="'tab' + (i + 1)" :class="{'this':curIndex === i}">
			<a href="javascript:void(0)" :class="[item.className, ((item.selectedClassName && curIndex === i) ? item.selectedClassName : '')]" @click="selectIndex(i)">{{ item.title }}</a>
		</li>
...
export default {
	...
	props:{
		...
		data:{
			type: Array,
			default: []
			//{title:'选项', className:'', selectedClassName:'', selected:()=>FunctionCode} selected返回false不移动column
		},
		column:{//
			type: String,
			default: 'column'
	...
	methods:{
		...
		selectIndex(index){
			...
			let moveColumn = true
			if (item.selected && $.isFunction(item.selected)) {
